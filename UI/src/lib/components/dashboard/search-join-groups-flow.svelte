<script lang="ts">
	...
	function handleSelectMovie(movie: GroupToJoin) {
		selectedGroupToJoin = movie;
			 ...
			 <div class="max-h-[calc(100vh-12rem)] overflow-y-auto p-4">
				...
				{:else}
					...
					{#if currentState === 'results'}
						<GroupSearchResults results={searchResults} searchQuery={searchQuery} userId={user?.id || ""} onSelect={handleSelectMovie} searchGroups={searchGroups}/>
					{/if}
