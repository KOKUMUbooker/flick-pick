<script lang="ts">
	...
	let _chatMsgsQuery = createInfiniteQuery<FetchedChatData, Error, InfiniteData<FetchedChatData>>(
		() => ({
	...
	let chatMsgsMutation = createMutation<{ message: string; msgId: string }, Error, CreateChatData>(
		() => ({
			mutationFn: async (data) => {
				return apiFetch(`${API_BASE_URL}/api/movie-night/${selectedEvent?.id}/chat`, {
					method: 'POST',
					headers: { 'Content-Type': 'application/json' },
		...
		<Card>
			<CardHeader class="border-b border-border bg-muted/30">
				<div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
					...
					<!-- RIGHT -->
					<Badge
						variant={getEventStatus(selectedEvent).variant as BadgeVariants}
				...
				<div class="border-t border-border p-4">
					<form
						...
						onsubmit={async (e) => {
							e.preventDefault();
							if (!selectedEvent)
								return toast.error('No active movie event in selection', { richColors: true });
